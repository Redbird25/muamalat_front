...
const EIMZOClient = {
  ...
  checkVersion: function (success, fail) {
    window.CAPIWS.version(function (event, data) {
  ...
  installApiKeys: function (success, fail) {
    global.CAPIWS.apikey(EIMZOClient.API_KEYS, function (event, data) {
      if (data.success) {
        success();
      } else {
        fail(null, data.reason);
  ...
  idCardIsPLuggedIn: function (success, fail) {
    ...
    if (!EIMZOClient.NEW_API2) {
      ...
      window.CAPIWS.callFunction({plugin: "idcard", name: "list_readers"}, function (event, data) {
        if (data.success) {
  ...
  loadKey: function (itemObject, success, fail, verifyPassword) {
    if (itemObject) {
      ...
      if (vo.type === "pfx") {
        ...
        window.CAPIWS.callFunction({plugin: "ftjc", name: "load_key", arguments: [vo.cardUID]}, function (event, data) {
          if (data.success) {
            const id = data.keyId;
            if (verifyPassword) {
              window.CAPIWS.callFunction({
  ...
  createPkcs7: function (id, data, timestamper, success, fail, detached, isDataBase64Encoded) {
    ...
    if (isDataBase64Encoded === true) {
      ...
      data64 = global.Base64.encode(data);
    }
    if (detached === true) {
      detached = 'yes';
    } else {
  ...
  _findTokens2: function (itemIdGen, itemUiGen, items, errors, callback) {
    let itmkey0;
    window.CAPIWS.callFunction({plugin: "ftjc", name: "list_all_keys", arguments: ['']}, function (event, data) {
      if (data.success) {
...
export const IMZOClient = EIMZOClient;
export const dates2 = dates;
